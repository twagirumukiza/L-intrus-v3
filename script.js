...
const CHAOS_NAMES={moving:'déplacement',splitMove:'sens opposés',blinking:'clignotement',staticNoise:'grésillement'};
...
function applyTrainingDifficulty(){
  ...
  if(lvl===30)return setBoardClasses(['moving','blinking'],'✦ Mouvement + clignotement renforcé');
  if(lvl===40){let pool=['moving','splitMove','blinking'],shuffled=pool.slice().sort(()=>Math.random()-.5),count=2+Math.floor(Math.random()*3),chosen=shuffled.slice(0,count);if(chosen.includes('splitMove')&&!chosen.includes('moving'))chosen.push('moving');return setBoardClasses(chosen,'🎲 '+chosen.map(c=>CHAOS_NAMES[c]).join(' + '))}
  if(lvl===50){let pool=['moving','splitMove','blinking'],shuffled=pool.slice().sort(()=>Math.random()-.5),count=1+Math.floor(Math.random()*3),chosen=['staticNoise',...shuffled.slice(0,count)];if(chosen.includes('splitMove')&&!chosen.includes('moving'))chosen.push('moving');setBoardClasses(chosen,'📺 '+chosen.map(c=>CHAOS_NAMES[c]).join(' + '));crackle();return}
  setBoardClasses([], '');
}
function applyDifficulty(){
  ...
  else{if(level>=10){board.classList.add('moving');label='↔ Déplacement alterné'}if(level>=20){board.classList.add('splitMove');label='⇆ Lignes en sens différents'}if(level>=25){board.classList.add('blinking');label='✦ Mouvement + clignotement'}if(level>=30){label='✦ Mouvement + clignotement renforcé'}}
...
function next(){
  ...
  for(let i=0;i<count;i++){
    ...
    let wrap=document.createElement('div');wrap.className='motionWrap';wrap.append(charEl(i===target?odd:normal));c.append(wrap);c.onclick=()=>pick(c,i);board.append(c)
  }
...
function revealIntruderAndEnd(){
  ...
  revealTimer=setTimeout(()=>{end(lastElapsed)},1500)
}
